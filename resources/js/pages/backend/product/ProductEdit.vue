...
<template>
  <AppLayout :breadcrumbs="breadcrumbItems">
    <section class="flex flex-col gap-1 mb-4 p-4">
      ...
      <div class="mt-2 mb-6">
        <p>圖片預覽：</p>
        <div v-if="subPreviewUrls" class="flex items-center gap-4 ml-4 my-2">
          <img  v-for="(url, index) in subPreviewUrls" :key="index" :src="url" class="w-[200px]"
            alt="輔助圖預覽">
        </div>
      </div>
      ...
      <div class="max-w-[1000px] flex justify-between mt-4">
        <button class="border border-black dark:border-gray-700 px-4 py-1 rounded-sm hover:bg-slate-300 dark:hover:bg-slate-700" type="button" @click="backBtn">
          返回
        </button>
        <button class="border border-black dark:border-gray-700 px-4 py-1 rounded-sm hover:bg-green-300 dark:hover:bg-green-950" type="button" @click="useAlert('確定要儲存嗎?', submit)">
          更新資料
        </button>
      </div>
